...
{
    public partial class clsVEdit : DevExpress.XtraBars.Ribbon.RibbonForm, IDXManagerPopupMenu
    {
        #region Fields
        ...
        public clsVEdit(DevExpress.XtraScheduler.SchedulerControl control, Appointment apt, bool openRecurrenceForm, List<Data.cResource> cResources, bool ReadOnly)
        {
            ...
            tbItemCode.Properties.DataSource = mstItemDB.GetMstitems();
            tbItemCode.Properties.DisplayMember = "ItemName";
            tbItemCode.Properties.ValueMember = "ItemCode";
            ...
            tbQty.EditValueChanged += (sender, e) =>
            {
                txtWorkTime.EditValue = (edtEndTime.Time - edtStartTime.Time).TotalMinutes / Convert.ToDouble(tbQty.EditValue);
            };
        }
        ...
        {
            if (appointment.CustomFields["ItemCode"] == null)
                tbItemCode.EditValue = "";
            else
            {
                tbItemCode.EditValue = appointment.CustomFields["ItemCode"];
                _ItemCode = appointment.CustomFields["ItemCode"].ToString();
            }
            edtResources.EditValue = appointment.ResourceId;
            if (appointment.CustomFields["Qty"] == null)
                tbQty.EditValue = "0";
            else
            {
                tbQty.EditValue = appointment.CustomFields["Qty"];
            }
            if (appointment.CustomFields["TimeWork"] == null)
                txtWorkTime.EditValue = "0";
            else
            {
                txtWorkTime.EditValue = appointment.CustomFields["TimeWork"];
            }

        }
        public virtual bool SaveFormData(Appointment appointment)
        {
            appointment.CustomFields["ItemCode"] = tbItemCode.EditValue;
            appointment.CustomFields["Qty"] = tbQty.EditValue;
            appointment.ResourceId = edtResources.EditValue;
            appointment.CustomFields["ItemName"] = tbItemCode.Text;
            appointment.CustomFields["TimeWork"] = txtWorkTime.EditValue;
            return true;
        ...
        private void tbItemCode_EditValueChanged(object sender, EventArgs e)
        {
            Controller.EditedAppointmentCopy.Location = (string)tbItemCode.EditValue;
        }
    }
}